...
function sample1(model::DiscretePGM, x::Vector, i::Int)
    post = a_posterior(model, x, i)
    return StatsBase.sample(StatsBase.WeightVec(post, 1.0))
end

function unif_random_assignment(model::DiscretePGM, clamps::Clamps)
...
function produce_chain!(model::DiscretePGM, clamps::Clamps, start::Vector=nothing)
    x = start
...
function sample(model::DiscretePGM, clamps::Clamps, burn_in::Int, interval::Int;
